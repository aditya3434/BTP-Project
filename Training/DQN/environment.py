...
class CarlaVehicle(object):
	...
	def reset(self,Norender):
		...
		#My code
		self.target_waypoint = carla.Transform(carla.Location(x = 231, y = 22, z=0), carla.Rotation(yaw=90))
		#For left turn
		...
		#My code
		init_loc = carla.Location(x = 176, y = 9.2, z=2)
		self.checkpoint = carla.Location(x = 215, y = 9.2, z=0)
		self.checkpoint_yaw = 0;
		#lane change
	...
	#My code
	def step_hybrid(self, action, ckpt):
		done = False
		self.vehicle.apply_control(carla.VehicleControl(throttle=action[0], steer=action[1], brake=action[2], reverse=action[3]))
		kmh = self.get_speed()
		reward = 0
		...
		if (ckpt == 0):
			print("Car angle : "+str(current_yaw))
			print("Checkpoint Angle : "+str(self.checkpoint_yaw))
			norm_target, diff_angle = self.compute_magnitude_angle(self.checkpoint, current_location, self.checkpoint_yaw, current_yaw)
			diff_angle = abs(current_yaw)
			print("Diff angle : "+str(diff_angle))
			kmh = self.get_speed()
			reward = reward + 0.1*kmh+0.1*(100-norm_target)
			if (diff_angle < 0.1):
				reward = reward + 10*(0.1-diff_angle)
			else:
				reward = reward - diff_angle
			...
			if norm_target<=4.2 and diff_angle<=0.1:
				...
				print("Diff angle : "+str(diff_angle))
				reward = reward-0.1*diff_angle-0.1*(norm_target)
